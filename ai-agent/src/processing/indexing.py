...
class VectorIndexBuilder:
    ...
    def build_index(
        self,
        transcript_chunks: List,  # Empty list for compatibility
        pdf_chunks: List[PDFChunk],
        embedding_engine: EmbeddingEngine
    ) -> None:
        """
        Build vector indices in OpenSearch from PDF chunks.
        
        This method:
        1. Creates PDF index if it doesn't exist
        2. Generates embeddings for all chunks
        3. Bulk indexes documents with embeddings and metadata
        
        Args:
            transcript_chunks: Empty list (kept for compatibility)
        ...
        """
        self.logger.info(
            f"Building indices with {len(pdf_chunks)} PDF chunks"
        )
        
        # Create PDF index if needed
        if pdf_chunks:
